"""
eval_with_fusion.py — SegEarth-OV-3 fusion mode evaluation

eval.py에 test_fusion_modes.py의 head fusion 전략을 추가한 평가 스크립트.
...
실행 예시:
  ...
  # baseline (max)
  python eval_with_fusion.py configs/cfg_loveda.py --fusion_mode max

  # heuristic (α=0.8 for building, α=0.2 for others)
  python eval_with_fusion.py configs/cfg_loveda.py \
      --fusion_mode heuristic --things_alpha 0.8 --stuff_alpha 0.2 \
      2>&1 | tee eval_fusion_heuristic.txt

  # entropy
  python eval_with_fusion.py configs/cfg_loveda.py \
      --fusion_mode entropy \
      2>&1 | tee eval_fusion_entropy.txt
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='SegEarth-OV-3 fusion mode evaluation (no adapter)')
...
def main():
    ...
    # ── 4. 결과 저장 ────────────────────────────────────────────────────────
    model_tag = cfg.model.model_type + f'+fusion_{args.fusion_mode}'
    ...
    if runner.rank == 0:
        tag = osp.basename(args.config).split('.')[0] + f'+fusion_{args.fusion_mode}'
